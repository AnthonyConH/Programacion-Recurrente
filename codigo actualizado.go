...
func LoadDataset(filename string) (Dataset, error) {
    ...
    if err != nil {
        return Dataset{}, fmt.Errorf("error al abrir el archivo: %v", err)
    }
    defer file.Close()
    ...
    if err != nil {
        return Dataset{}, fmt.Errorf("error al leer el archivo: %v", err)
    }

    // Omitir la primera fila (cabecera)
    for i, record := range records[1:] { // i es el índice de la fila, excluyendo la cabecera
        // Convertir las columnas relevantes a características
        x := make([]float64, 3) // Cambié a 3 características: Edad, Sexo, id_ubigeo

        // Edad
        x[0], err = strconv.ParseFloat(record[1], 64) // Edad
        if err != nil {
            return Dataset{}, fmt.Errorf("error al convertir la edad en la línea %d: %v", i+2, err)
        }

        // Sexo (convertido a 0 para M y 1 para F)
        if strings.ToUpper(record[3]) == "F" {
            x[1] = 1
        } else {
            x[1] = 0
        }

        // id_ubigeo (convertir a float64)
        x[2], err = strconv.ParseFloat(record[4], 64) // id_ubigeo
        if err != nil {
            return Dataset{}, fmt.Errorf("error al convertir id_ubigeo en la línea %d: %v", i+2, err)
        }

        // Leer la columna de Diagnóstico como etiqueta (ahora convertimos a float64)
        y, err := strconv.ParseFloat(record[7], 64) // Diagnóstico (CIE-10)
        if err != nil {
            return Dataset{}, fmt.Errorf("error al convertir el diagnóstico en la línea %d: %v", i+2, err)
        }

        data.X = append(data.X, x)
        data.Y = append(data.Y, int(y)) // Si prefieres que la etiqueta sea un entero
    }
...
// Función principal para cargar datos, entrenar el modelo Random Forest, y aceptar inputs del usuario
func main() {
	start := time.Now()

	// Cargar el dataset desde el archivo TB_DIGTEL_ANEMIA_TAMIZAJE.csv
	data, err := LoadDataset("dataset_limpio.csv")
	if err != nil {
		log.Fatalf("Error al cargar el dataset: %v", err)
	}

	// Solicitar al usuario el número de árboles y la profundidad máxima
	var numTrees int
	var maxDepth int
	...
	fmt.Print("Ingrese la profundidad máxima de los árboles: ")
	fmt.Scan(&maxDepth)

	// Crear y entrenar el modelo Random Forest
	rf := &RandomForest{}
	rf.Train(data, numTrees, maxDepth)
